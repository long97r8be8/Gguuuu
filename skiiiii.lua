...
local MyVersion = "4.3.1"

print("LongHuyFnf using version: ".. MyVersion)

if _G.UsingHub then
	return
end

if not isfile("LongHuyFnfVersion.json") then
	writefile("LongHuyFnfVersion.json",HttpService:JSONEncode({["Version"]=VersionHub}))
else
	local _Version = readfile("LongHuyFnfVersion.json")
	pcall(function()
	...
	if _Version then
		...
		if _Version.Version ~= MyVersion then
			warn("You are using out version LongHuyFnf!")
			...
			TextLabel.Text = "You are using out version LongHuyFnf! You do want use old version? [Chat /e yes or /e no]"
			...
			if no then
				...
				warn("Copied Script Latest Version LongHuyFnf!")
				wait(1)
				TextLabel.Text = "Copied Script Lasest Version LongHuyFnf!"
				wait(1.5)
...
pcall(function()
	...
	TweenService:Create(TextLabel,TweenInfo.new(1.5),{TextTransparency = 0}):Play()
	wait(3)
	TextLabel.Text = "Thank you using the LongHuyFnf Version ".. MyVersion .."!"
	wait(1.5)
	TextLabel.Text = "CauBeBungCaC Con CăC!"
	wait(1.5)
	TweenService:Create(TextLabel,TweenInfo.new(1),{TextTransparency = 1}):Play()
	wait(1.2)
	Frame.Size = UDim2.new(1,0,0.05,0)
...
elseif PlaceId == 6604417568 then
	local X = Material.Load({
		Title = "LongHuyFnf [DONT PLAY THIS GAME]",
		Style = 3,
		SizeX = 500,
		SizeY = 350,
		Theme = "Light",
